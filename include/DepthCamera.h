 ...
 * Any depth camera should be able to generate a XYZMap, AmpMap (confidence),
 * and FlagMap.
 */
class DepthCamera {
   ...
   public:
    ...
    CameraIntrin getIntrinsics() const;
   ...
   private:
     ...
     * call, buffers are swapped if false, a dummy mat is created
     * @param img pointer to foreground image
     * @param buf pointer to back buffer
     */
    void swapBuffer(bool (DepthCamera::*check_func)() const, cv::Mat &img,
                    cv::Mat &buf);
     ...
     * and FlagMap.
     */
    static void removeNoise(cv::Mat &xyzMap, cv::Mat &ampMap,
    ...
    std::atomic<bool> captureInterrupt;
