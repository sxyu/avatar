...
inline void fromSpherical(double rho, double theta, double phi,
                          Eigen::Vector3d& out) {
...
template <class T>
inline void paintTriangleBary(cv::Mat& output, const cv::Size& image_size,
 ...
 */
template <class T>
inline void paintTriangleSingleColor(cv::Mat& output_image,
...
AvatarModel::AvatarModel(const std::string& model_dir,
    ...
    if (exists(npzPath)) {
        ...
        jointRegressor = util::loadFloatMatrix(jreg_raw, n_joints, n_verts)
                             .transpose()
                             .sparseView();
        initialJointPos = baseCloud * jointRegressor;
...
void Avatar::update() {
    ...
    if (model.useJointShapeRegressor) {
        ...
        Eigen::Map<Eigen::VectorXd> jointPosVec(jointPos.data(),
                                                3 * model.numJoints());
        jointPosVec.noalias() =
            model.jointShapeRegBase + model.jointShapeReg * w;
    } else {
