...
//
//     // Body pose control definitions (currently this control system only
//     // supports rotation along one axis per body part)
//     const std::vector<std::string> CTRL_NAMES = {
//         "L HIP",   "R HIP",   "L KNEE",  "R KNEE",  "L ANKLE",
//         "R ANKLE", "L ELBLW", "R ELBOW", "L WRIST", "R WRIST",
//         "HEAD",    "SPINE2",  "ROOT"};
//     const std::vector<int> CTRL_JNT = {
//         SmplJoint::L_HIP,      SmplJoint::R_HIP,   SmplJoint::L_KNEE,
//         SmplJoint::R_KNEE,     SmplJoint::L_ANKLE, SmplJoint::R_ANKLE,
//         SmplJoint::L_ELBOW,    SmplJoint::R_ELBOW, SmplJoint::L_WRIST,
//         SmplJoint::R_WRIST,    SmplJoint::HEAD,    SmplJoint::SPINE2,
//         SmplJoint::ROOT_PELVIS};
//     const std::vector<Eigen::Vector3d> CTRL_AXIS = {
//         AXISX, AXISX, AXISX, AXISX, AXISX, AXISX, AXISY,
//         AXISY, AXISY, AXISY, AXISX, AXISX, AXISY};
...
//         cv::createTrackbar("PC" + std::to_string(i), "Body Shape", &pcw[i],
//                            2000);
//     }
//
...
//         [&interrupt](const pcl::visualization::KeyboardEvent& evt) {
...
//             viewer->addPointCloud<pcl::PointXYZ>(ark::avatar_pcl::getCloud(ava),
//                                                  "vp1_cloudHM", vp1);
...
//                 if (angle == 0)
//                     ava.r[CTRL_JNT[i]].setIdentity();
//                 else
//                     ava.r[CTRL_JNT[i]] = Eigen::AngleAxisd(angle,
//                     CTRL_AXIS[i])
//                                              .toRotationMatrix();
//             }
//
...
//                 cv::setTrackbarPos("PC" + std::to_string(k), "Body Shape",
//                                    pcw[k]);
//             }
//
//             double prior =
//                 ava.model.posePrior.residual(ava.smplParams()).squaredNorm();
//             // show pose prior value
//             if (!viewer->updateText("-log likelihood: " +
//             std::to_string(prior),
//                                     10, 20, 15, 1.0, 1.0, 1.0,
//                                     "poseprior_disp")) {
//                 viewer->addText("-log likelihood: " + std::to_string(prior),
//                 10,
//                                 20, 15, 1.0, 1.0, 1.0, "poseprior_disp");
//             }
//         }
...
//     }
//
//     auto viewer = pcl::visualization::PCLVisualizer::Ptr(new
//     pcl::visualization::PCLVisualizer("3D Viewport"));
//
//     viewer->initCameraParameters();
//     int vp1 = 0;
//     viewer->setWindowName("3D View");
...
//
//     volatile bool interrupt = false;
//     viewer->registerKeyboardCallback([&interrupt](const
//     pcl::visualization::KeyboardEvent & evt) {
//         unsigned char k = evt.getKeyCode();
//         if (k == 'Q' || k == 'q' || k == 27) {
//             interrupt = true;
//         }
...
//
//     std::cerr << "BEFORE\n";
//     ViconSkeleton skel("/mnt_d/DataSets/human/CMUMocap/mocapPlayer/07.asf",
//     "/mnt_d/DataSets/human/CMUMocap/mocapPlayer/07_05-walk.amc", 1);
//     std::cerr << "LOAD\n";
//
//     bool controlsChanged = false;
//     while (!interrupt) {
...
//                 viewer->addText3D(std::to_string(i), curr,
//                 0.03, 1.0, 1.0, 1.0, jointName + "T", 0);
//             }
...
//
//             //viewer->removePointCloud("vp1_cloudHM");
//             //viewer->addPointCloud<pcl::PointXYZ>(avatar_pcl::getCloud(ava),
//             "vp1_cloudHM", vp1);
//             // viewer->removePolygonMesh("meshHM");
//
//             // auto mesh = ark::avatar_pcl::getMesh(ava);
...
//             //     else ava.r[CTRL_JNT[i]] = Eigen::AngleAxisd(angle,
//             CTRL_AXIS[i]).toRotationMatrix();
//             // }
//
//             for (int i = 1; i < (int)pcw.size(); ++i) {
//                 ava.w[i] = (float)(pcw[i] - 1000) / 500.0;
...
//             // // faceIndices.resize(std::unique(faceIndices.begin(),
//             faceIndices.end()) - faceIndices.begin());
...
//
//             auto visualCloud =
//             boost::shared_ptr<pcl::PointCloud<pcl::PointXYZRGBA> >(new
//             pcl::PointCloud<pcl::PointXYZRGBA>()); for (size_t i = 0; i <
//             ava.cloud.cols(); ++i) {
//                 // if (pointVisible[i]) {
//                     pcl::PointXYZRGBA pt;
//                     pt.x = ava.cloud(0, i);
...
//             viewer->addPointCloud<pcl::PointXYZRGBA>(visualCloud,
//             "vp1_cloudHMC");
//             // ava.visualize(viewer, "vp1_", vp1);
//             //
