...
struct KDTreeEigenColMajorMatrixAdaptor {
  ...
  inline void query(const num_t *query_point, const size_t num_closest,
    ...
    nanoflann::KNNResultSet<typename MatrixType::Scalar, IndexType> resultSet(
        num_closest);
...
public:
  ...
  bool Plus(const double *x_ptr, const double *delta,
    ...
    if (norm_delta > 0.0) {
      ...
      Eigen::Quaterniond delta_q(cos(norm_delta), sin_delta_by_delta * delta[0],
...
struct AvatarEvaluationCommonData : public ceres::EvaluationCallback {
  ...
  AvatarEvaluationCommonData(AvatarOptimizer &opt, bool shape_enabled = false)
      : opt(opt), ava(opt.ava), numJointSpaces(opt.ava.model.numJoints() + 1),
    ...
    if (shape_enabled) {
      ...
      if (ava.model.useJointShapeRegressor) {
        ...
        for (int i = 0; i < ava.model.numShapeKeys(); ++i) {
          ...
          Eigen::Map<const CloudType> keyCloud(
              ava.model.keyClouds.data() + i * ava.model.numPoints() * 3, 3,
              ava.model.numPoints());
          for (int j = 0; j < ava.model.numJoints(); ++j) {
      ...
      for (int j = 1; j < ava.model.numJoints(); ++j) {
        if (j)
          Sp[j].noalias() = S[j] - S[ava.model.parent[j]];
      }
      Sp[0].setZero();
      H[0].setZero();
    }
  ...
  void PrepareForEvaluation(bool evaluate_jacobians,
    ...
    if (new_evaluation_point) {
      ...
      for (int i = 0; i < numJointSpaces - 1; ++i) {
        ...
        localJacobian[i] << x(3), x(2), -x(1), -x(2), x(3), x(0), x(1), -x(0),
            x(3), -x(0), -x(1), -x(2);
      }
      // PROFILE(lojac);
      ...
      if (shapeEnabled) {
        ...
        for (int j = 1; j < numJointSpaces - 1; ++j) {
          H[j].noalias() =
              R(-1, ava.model.parent[j]) * Sp[j] + H[ava.model.parent[j]];
        }
      }
      // PROFILE(H);
  ...
  struct Ancestor {
    Ancestor() {}
    explicit Ancestor(int jid, int assigned = -1, double assign_weight = 0.0)
        : jid(jid) {
      if (assigned >= 0) {
        assign[0] = assigned;
  ...
  bool getICPJacobians(double *residuals, double **jacobians) const {
    ...
    if (jacobians != nullptr) {
      ...
      if (commonData.shapeEnabled && jacobians[i + 1] != nullptr) {
        Eigen::Map<Eigen::Matrix<double, 3, Eigen::Dynamic, Eigen::RowMajor>> J(
            jacobians[i + 1], 3, ava.model.numShapeKeys());
    ...
    if (compute_jacobians) {
      ...
      for (size_t i = 0; i < commonData.ancestor[pointId].size(); ++i) {
        ...
        dRot << u(1) * v(1) + v(2) * u(2),
            ...
            v(2) * u(2) + u(0) * v(0), w * v(0) + u(1) * v(2) - 2 * v(1) * u(2),
            v(0) * u(2) - u(0) * v(2),
...
struct AvatarICPCostFunctor : ceres::CostFunction {
  AvatarICPCostFunctor(
      ...
      : commonData(common_data), cacheId(cache_id), dataCloud(data_cloud),
...
struct AvatarPosePriorCostFunctor : ceres::CostFunction {
  ...
  bool Evaluate(double const *const *parameters, double *residuals,
    ...
    int compIdx;
    resid.noalias() =
        posePrior.residual(smplParams, &compIdx) * commonData.scaledBetaPose;
    if (jacobians != nullptr) {
      ...
      for (int i = 0; i < nSmplJoints; ++i) {
        if (jacobians[i] != nullptr) {
          ...
          J.topLeftCorner<Eigen::Dynamic, 3>(nResids - 1, 3).noalias() =
              L.middleRows<3>(i * 3).transpose() * 0.707106781186548 *
  ...
  bool Evaluate(double const *const *parameters, double *residuals,
    ...
    if (jacobians != nullptr) {
      if (jacobians[0] != nullptr) {
        ...
        J.noalias() =
            Eigen::MatrixXd::Identity(numShapeKeys, numShapeKeys) * betaShape;
      }
    }
    return true;
  }
...
struct AvatarICPAutoDiffCostFunctor {
  ...
  bool operator()(T const *const *params, T *residual) const {
    ...
    Eigen::Matrix<T, 3, Eigen::Dynamic> cloud(3,
    ...
    if (commonData.shapeEnabled) {
      ...
      cloudVec.noalias() = commonData.ava.model.keyClouds.cast<T>() * wMap +
    ...
    for (auto &assign : commonData.ava.model.assignedJoints[pointId]) {
      Eigen::Matrix<T, 3, 1> vec =
          (cloud.col(pointId) - jointPos.col(assign.second)).template cast<T>();
      for (int p = assign.second; p != -1; p = commonData.ava.model.parent[p]) {
        ...
        if (p)
          vec.noalias() +=
              jointPos.col(p) - jointPos.col(commonData.ava.model.parent[p]);
      }
...
void findNN(const CloudType &data_cloud,
  ...
  if (invert) {
    ...
    for (int i = 0; i < data_cloud.cols(); ++i) {
      ...
      if (modelPartKD[partId] == nullptr)
        continue;
      modelPartKD[partId]->index->findNeighbors(
          resultSet, data_cloud.data() + i * 3, nanoflann::SearchParams(10));
    ...
    for (int i = 0; i < model_cloud.cols(); i += nn_step) {
      ...
      if (kd_tree) {
        kd_tree->index->findNeighbors(resultSet, model_cloud.data() + i * 3,
...
void debugVisualize(
  ...
  auto dataPclCloud = pcl::PointCloud<pcl::PointXYZRGBA>::Ptr(
      new pcl::PointCloud<pcl::PointXYZRGBA>());
  // auto matchedModelPointsCloud = pcl::PointCloud<pcl::PointXYZRGBA>::Ptr(new
  ...
  curr.y << "," << curr.z << "\n"; cv::Vec3f colorf(0.f, 0.f, 1.0f); std::string
  jointName = "avatarJoint" + std::to_string(i); viewer->removeShape(jointName,
  0); viewer->addSphere(curr, 0.02, colorf[0], colorf[1], colorf[2], jointName,
  0);
      // if (joints[i]->parent) {
  ...
  */
  ...
  for (size_t i = 0; i < correspondences.size(); ++i) {
    for (size_t j = 0; j < correspondences[i].size(); ++j) {
      ...
      viewer->addLine<pcl::PointXYZ, pcl::PointXYZ>(p2, p1, 0.0, 1.0, 0.0, name,
                                                    0);
    }
  }
...
void testCompareAutoDiff(AvatarOptimizer &opt, const ark::CloudType &data_cloud,
  ...
  for (int i = 0; i < params.size(); ++i) {
    int cols = 3 + (i > 0);
    if (i + 1 == params.size())
      cols = opt.ava.model.numShapeKeys();
    std::cerr << "BLOCK " << i;
    if (i > 0 && (i < params.size() - 1 || !common.shapeEnabled))
      std::cerr << " -> " << common.ancestor[model_point_id][i - 1].jid + 1;
    std::cerr << "\n";
    for (int j = 0; j < 3; ++j) {
...
AvatarOptimizer::AvatarOptimizer(Avatar &ava, const CameraIntrin &intrin,
    ...
    : ava(ava), intrin(intrin), imageSize(image_size), numParts(num_parts),
      partMap(part_map) {
...
void AvatarOptimizer::optimize(
  ...
  for (int icp_iter = 0; icp_iter < icp_iters; ++icp_iter) {
    ...
    /** Scale the function weights according to number of ICP type residuals.
     * Otherwise the function terms become extremely imbalanced in some cases.
     */
    ...
    if (betaPose > 0.) {
      problem.AddResidualBlock(new AvatarPosePriorCostFunctor(common), NULL,
                               posePriorParams);
    }
    if (betaShape > 0.) {
      problem.AddResidualBlock(
