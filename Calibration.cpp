...
namespace ark {
    ...
    CameraIntrin::CameraIntrin(const std::string& path) {
        readFile(path);
    }

    CameraIntrin::CameraIntrin(const cv::Mat& xyz_map) {
        inferFromXYZ(xyz_map);
    }

    CameraIntrin::CameraIntrin(const cv::Vec4d& intrin) {
        _setVec4d(intrin);
    }
    ...
    void CameraIntrin::_setVec4d(const cv::Vec4d& intrin) {
        fx = intrin[0]; cx = intrin[1];
        fy = intrin[2]; cy = intrin[3];
        std::fill(k, k + 6, 0.f);
        std::fill(p, p + 2, 0.f);
    } 
    ...
    cv::Mat CameraIntrin::depthToXYZ(const cv::Mat& depth) const {
        ...
        const float * inPtr; cv::Vec3f * outPtr;
